/*
 * This simple shell project was completed by John Burrell and Ben Tetreault.
 */
...
static void doPipe(char** p1Args, char** line, int* lineIndex) {
    ...
    if (CHILD_PID(pid)) { /* Child -- will execute left-hand-side process */
        ...
        dup2(pipefd[1], 1); //Takes stdout and replaces it with the pipe out 
        close(1); //Closes standard out on the left side
        
       

        execvp(pipefd[1], p1Args);
        childPid = forkWrapper();
